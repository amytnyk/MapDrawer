...
vector<Color> GetColors(vector<int> numbers, vector<Color> allcolors)
{
	...
	while (window.isOpen())
	{
		...
		window.clear();
		DrawText(100, 100, 40, "Founded numbers:", Color::Green, false);
		...
		for (size_t i = 0; i < numbers.size(); i++)
		{
			DrawText(100, 100 + (i + 1) * 50, 40, to_string(numbers[i]), Color::Magenta, false);
			RectangleShape rectangle;
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			if (event.text.unicode < 128)
			{
				...
				{
					if (event.text.unicode == '\b') // return key
					{
						if (path.size() != 0)
						{
		...
		window.clear();
		DrawText(100, 100, 40, path, Color::Red, false);
		window.display();
	}
	return path;
}
...
int main()
{
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			if (Mouse::isButtonPressed(Mouse::Button::Left))
			{
				for (size_t i = 0; i < tmap.size(); i++)
				{
					for (size_t j = 0; j < tmap[i].size(); j++)
					{
						if (IsOnSquare(j * 30, i * 30, 30, 30, position.x, position.y))
						{
							if (last == 0)
							{
		...
		window.clear();
		//DrawBackground();
		DrawMap(tmap, colors, numbers); // Error
		window.display();
	}
